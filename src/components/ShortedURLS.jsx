...
const ShortedURLS = ({ state }) => {
  ...
  return (
    <div className={`${state.urls.length > 1 && "overflow-y-scroll"} border-slate-900 border mt-4 bg-[#17191b] focus:ring-blue-900 max-h-64`}>

      {state.urls.length > 0 
      ? (
        state.urls.map(item => {
          return (
              <article className=" text-slate-200 pb-2 pt-2 border-b-slate-900" key={item.UUIID}>
                  <h3>{item.url}</h3>
